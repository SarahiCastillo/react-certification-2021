...
import { routeName, routeId, apiResponseVideo } from './mocks/api.mocks';
import '@testing-library/jest-dom/extend-expect';

describe('API', () => {
  it('calls the API with search value', async () => {
    const resultApi = await API.get(routeName);
    const result = {
      kind: 'youtube#searchListResponse',
      etag: '2bNauqk9oCK2cqPy857JwGZU1So',
      ...
      items: [
        {
          ...
          snippet: {
            ...
            thumbnails: {
              default: {
                url:
                  'https://yt3.ggpht.com/a/AATXAJwYxtvHiokGQH_SOjK3yD21e9JOty04ChmU33rtFA=s88-c-k-c0xffffffff-no-rj-mo',
              },
              medium: {
                url:
                  'https://yt3.ggpht.com/a/AATXAJwYxtvHiokGQH_SOjK3yD21e9JOty04ChmU33rtFA=s240-c-k-c0xffffffff-no-rj-mo',
              },
              high: {
                url:
                  'https://yt3.ggpht.com/a/AATXAJwYxtvHiokGQH_SOjK3yD21e9JOty04ChmU33rtFA=s800-c-k-c0xffffffff-no-rj-mo',
              },
            },
    ...
    resultApi.etag = '2bNauqk9oCK2cqPy857JwGZU1So';
    resultApi.pageInfo.totalResults = 2086;
    await expect(resultApi).toEqual(result);
    expect(resultApi).toBeCalledTimes(1);
  });

  it('calls the API with video ID', async () => {
    const resultApiId = await API.get(routeId);
    const mockFn = jest.fn(() => {
      const resultApi = {
        kind: 'youtube#searchListResponse',
        etag: 'IOhPZmJ5V7cUB7tYcrvO63LQ-3M',
        regionCode: 'MX',
        pageInfo: {
          totalResults: 1,
          resultsPerPage: 5,
    ...
    await expect(resultApiId).toEqual(apiResponseVideo);
    expect(mockFn).toBeCalledTimes(1);
  });

  it('calls the API with video ID', async () => {
    const resultApiId = await API.getEntity(
      'v3/search?part=snippet&key=AIzaSyDzRXfcJrid0u60yOqHIGWvwmqVDUhMHhE&q=',
      'HYyRZiwBWc8'
    );
    await expect(resultApiId).toEqual(apiResponseVideo);
    expect(resultApiId).toBeCalledTimes(1);
  });
});
