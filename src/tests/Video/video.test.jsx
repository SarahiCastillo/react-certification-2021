...
import { routeId } from '../API/mocks/api.mocks';
import '@testing-library/jest-dom/extend-expect';

describe('Video', () => {
  it('returns loading when data is empty', () => {
    const apiResponseVideo = '';

    const { container } = render(
      <AuthProvider>
        <UseSearch>
          <IntlProvider locale="en">
            <Detail data={apiResponseVideo} />
          </IntlProvider>
        </UseSearch>
      </AuthProvider>
    );
    ...
    expect(container.outerHTML).toEqual(
      '<div><section class="sc-dlnjPT gUShcZ"><article class="sc-hKFyIo ghSTqJ">Loading...</article></section></div>'
    );
  });
  ...
  it('renders a video when data contains API reponse', async () => {
    const resultApiId = await API.get(routeId);
    const isLoading = false;
    const data = resultApiId.items;
